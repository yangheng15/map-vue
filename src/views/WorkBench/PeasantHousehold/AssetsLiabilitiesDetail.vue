...
export default {
  ...
  methods: {
    ...
    async editRecord(val) {
      ...
      this.familyCode = res.data.familyCode;
      this.customerImgTime = res.data.assetsImgList;
      if (res.data.assetsImgList) {
    ...
    modifyResult() {
      ...
      if (this.assets_type_txt == "资产" && this.asset_name_txt == undefined) {
        Dialog.alert({
          title: "提示",
          message: "请输入资产名称！",
        });
  ...
  filters: {
    customers_assets_type(val) {
      const findWill = JSON.parse(
        localStorage.getItem("dicCustomersAssetsType")
      ).find((it) => +it.key == val);
      return findWill ? findWill.value : "";
    },
    customers_assets_type_info(val) {
      const findWill = JSON.parse(
        localStorage.getItem("dicAssetsTypeInfo")
      ).find((it) => +it.key == val);
      return findWill ? findWill.value : "";
    },
    customers_liabilities_type_info(val) {
      const findWill = JSON.parse(
        localStorage.getItem("dicLiabilitiesTypeInfo")
      ).find((it) => +it.key == val);
      return findWill ? findWill.value : "";
    },
  },
