<template>
    <div class="grid">
        ...
        <van-search class="positionFixed" v-model="searchVal" placeholder="查找地点" @clear="onSearchCancel">
            <!-- <template #action>
        ...
        <div class="search-box" v-show="searchShow">
            <van-list class="local-list">
                <van-cell @click="localOnclick(item)" v-for="item in localSearchList" :key="item.title">
                    <div>
                        {{ item.title }}
                        <span style="font-size: 12px; margin-left: 5px">电话：{{ item.phoneNumber }}</span>
                    </div>
        ...
        >
          ...
          <p>剩余日期：30天</p>
            ...
            <template>
                ...
                >
                    <bm-label :content="item.name" :labelStyle="{ color: 'red', fontSize: '14px' }" :offset="{ width: -35, height: 30 }" />
                </bm-marker>
            </template>
      ...
      ></bm-geolocation> -->
        ...
        <div v-show="isPopupVisibleSign" class="isPopupVisibleSign">
            <van-form class="isPopupVisibleSign_content" @submit="onSubmit">
                ...
                <van-field required v-model="signData.sign_name" name="name" label="名称：" placeholder="请填写名称" :rules="[{ required: true }]" />
                <van-field v-model="signData.sign_phone" name="telphone" label="电话：" placeholder="请填写电话" />
                <van-field v-model="signData.sign_address" name="address" label="地址：" placeholder="请填写地址" />
                <van-field readonly v-model="signData.sign_position" name="position" label="位置：" placeholder="请填写位置" />
                <van-field
                    required
                    :rules="[{ required: true }]"
                    clickable
                ...
                <van-popup v-model="resource_type" position="bottom">
                    <van-picker show-toolbar :columns="resource_type_list" @confirm="onResource_type" @cancel="resource_type = false" />
                </van-popup>
                <!-- <van-field
          readonly
          clickable
        ...
        <van-popup v-model="marked_or_not" position="bottom">
          ...
          />
                ...
                <div style="margin: 10px 0px" class="save">
                    ...
                    <van-button round type="primary" @click="isPopupVisibleSign = false">取消</van-button>
                </div>
            </van-form>
        </div>
    ...
    methods: {
        ...
        createInfoWindow(map, data) {
            ...
            for (var i = 0; i < data_info.length; i++) {
                var marker = new BMap.Marker(new BMap.Point(...data_info[i]['position'].split(','))); // 创建标注
        ...
        dic_grid_resource_type(val) {
            const findWill = JSON.parse(localStorage.getItem('dicGridResource')).find((it) => +it.key == val);
        ...
        showTypeIds(data) {
            ...
            const positionArr = this.typeIdsItem.position && this.typeIdsItem.position.split(',');
        ...
        async onSubmit(values) {
            ...
            const code = this.resource_type_txt && this.filterData.find((it) => it.codeText === this.resource_type_txt)['code'];
            this.$httpPost({
